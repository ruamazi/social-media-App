...
const ActionBar = ({ post: post }) => {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (!liked) {
        // adding id of current user to post.likes array
        ...
        // removing id of current user from post.likes array
        const updatedPosts = posts.map((p) => {
          if (p._id === post._id) {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setPosts(updatedPosts);
      showToast("Succuss", "Your comment is added.", "success");
      onClose();
      setReplyValue("");
    } catch (error) {
      showToast("Error", error, "error");
  ...
  return (
    <Flex flexDirection={"column"} gap={2}>
      ...
      <Flex gap={"2"} alignItems={"center"}>
        ...
        <Box width={"1"} height={"1"} borderRadius={"50%"} bg={"gray.light"} />
        <Text color={"gray.light"} fontSize={"sm"}>
